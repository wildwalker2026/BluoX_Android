...
| 功能 | 说明 |
|------|------|
| 🤖 **多 AI 平台聚合** | 支持千问、DeepSeek、豆包、智谱、MiniMax、Kimi 等主流大模型 |
...
### v1.3.12
- 🆕 Skill 系统上线 — 动态加载可执行工具和参考文档，引入 CLI 模式，支持禁用/启用
- 🆕 工具调用详情弹窗支持 git diff 可视化渲染
- 🆕 支持自定义数据目录前缀，笔记/Skills/日志/备份路径均可动态配置
- 🆕 解除 DeepSeek thinking 与 tools 互斥限制，两者可同时开启
- 🐛 版本保存 tool_calls 来源修正
- 🐛 级联删除版本匹配修正
- 🎨 底部菜单选中状态改用 isActive 样式高亮，移除 ✓ 标识
- 🎨 调整消息操作按钮布局及删除逻辑
- ⚡ 版本号升级至 1.3.12
